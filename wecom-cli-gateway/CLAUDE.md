...
- **停止执行 `/stop`**：命中 `/stop` / `停止` / `停止执行` 时终止该用户**全部** cliType 下**正在执行**的会话并返回停止提示。靠 `running` 注册表（`Map<key,{session,stopped}>`）在**锁检查之前**遍历 kill() 正在执行的会话；被终止的会话检测 `stopped` 标记后原消息显示"已停止执行"、不回写 session。命令词可配置 `stopCommands`。
